...
#
#
# IF(AND(OR('brother (?x) (?y)', 'sister (?x) (?y)' ),
#        OR('son (?z) (?x)', 'daughter (?z) (?x)'),
#        OR('son (?f) (?y)', 'daughter (?f) (?y)')),
#     THEN('cousin (?z) (?f)')
#    )
#
# IF(AND('parent (?x) (?y)',
...
family_rules = [
    ...
    ,
    # make sure that brothers and sisters are sons and daughters of both parents
    IF(AND(OR('brother (?x) (?y)', 'sister (?x) (?y)'),
           'parent (?z) (?x)',
           'parent (?f) (?y)',
           'female (?y)',
