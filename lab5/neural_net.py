...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        for i in range(len(self.my_inputs)):
            result += self.my_inputs[i].output() * self.my_weights[i].get_value()
    ...
    def compute_doutdx(self, elem):
        ...
        """
        # Weight directly connected to the current neuron
        if self.has_weight(elem):
            # get index of 'elem' in 'self.weights'
            for i, w in enumerate(self.my_weights):
                if w == elem:
                    index = i
            return self.output() * (1 - self.output()) * self.my_inputs[index].output()

        ans = 0
        # Weight not directly connected to the current neuron
        for i in range(len(self.my_inputs)):
            ans += self.my_weights[i].get_value() * self.my_inputs[i].dOutdX(elem)

        return self.output() * (1 - self.output()) * ans
        # raise NotImplementedError, "Implement me!"
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        desired = self.my_desired_val
        output = self.my_input.output()
...
def make_neural_net_challenging():
    ...
    w1A = Weight('w1A', 6.843323)
    w2A = Weight('w2A', -4.500452)
    w1B = Weight('w1B', 2.569573)
    w2B = Weight('w2B', -5.517072)
    w2C = Weight('w2C', 3.388125)
    w1C = Weight('w1C',  0.731263)

    wA = Weight('wA', 2.010715)
    wB = Weight('wB', -3.798775)
    wC = Weight('wC',  14.718200)
    wD = Weight('wD',  3.706422)
    wAD = Weight('wAD', 7.802604)
    wBD = Weight('wBD', -8.949819)
    wCD = Weight('wCD', -9.448479)
